...
async function doChoreography(page, canvas, steps) {
  for (const step of steps) {
    // Handle click_selector if specified
    if (step.click_selector) {
      try {
        await page.click(step.click_selector, { timeout: 5000 });
        await page.waitForTimeout(100);
      } catch (err) {
        console.warn("Failed to click selector", step.click_selector, err.message);
      }
    }
    ...
    for (const button of buttons) {
      if (button === "left_mouse_button" || button === "right_mouse_button") {
        ...
        mouseMoved = true;
        // Trigger mousemove event on canvas for the game to update hoverTile
        ...
        await sleep(50);
        // Use native mouse down
        await page.mouse.down({ button: button === "left_mouse_button" ? "left" : "right" });
        // Also trigger mousedown event on canvas for the game to register the click
        await page.evaluate(({ mx, my }) => {
          const canvasEl = document.getElementById('game-canvas');
          if (canvasEl) {
            const rect = canvasEl.getBoundingClientRect();
    ...
    // If no mouse button but mouse position specified, just move
    if (!mouseMoved && typeof step.mouse_x === "number" && typeof step.mouse_y === "number") {
      const bbox = canvas ? await canvas.boundingBox() : null;
      if (bbox) {
        ...
        }, { mx: step.mouse_x, my: step.mouse_y });
        await sleep(50);
      }
    }
    ...
    // Release buttons and trigger mouseup on canvas
    for (const button of buttons) {
      if (button === "left_mouse_button" || button === "right_mouse_button") {
        await page.mouse.up({ button: button === "left_mouse_button" ? "left" : "right" });
        // Also trigger mouseup event on canvas
        if (mouseMoved) {
          await page.evaluate(({ mx, my }) => {
            const canvasEl = document.getElementById('game-canvas');
            if (canvasEl) {
